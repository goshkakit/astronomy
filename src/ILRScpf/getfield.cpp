...
*/
/*  Changes:
	09/10/93 - Lynx - allow for '+' sign. rlr.
*/
//int
...
int getfield( char *buffer, int pos, int fieldsize, double* value)
{
	static char SccsId[] = "@(#)getfield.c	1.1\t12/06/90";
	//int decDigits;
	int c, i, sawDecPoint, sawMinus, numblanks;
	...
	for (i=pos; i<fieldsize+pos; ++i) {
		...
		if (c >= '0' && c <= '9') {
			...
			sawDecPoint = 1;
		} else if (c == '+') {	/* should actually check 'saw sign' */
			if (sawExponent) {
...
int getifield( char *buffer, int pos, int fieldsize, long* value)
{
	//int decDigits;
	int c, i, sawMinus, numblanks;
	long intval;
